...
const validateUser = async (email, clave) => {
    ...
    const cliente = await pool.query(`SELECT contraseña_cliente, email_cliente as email, nombre_cliente as nombre, id_cliente as id  FROM cliente WHERE email_cliente = '${email}';`);

    if (cliente.rows == '') {
        const trabajador = await pool.query(`SELECT contraseña_trabajador, nombre_trabajador as nombre,  email_trabajador as email, id_trabajador as id  FROM trabajador WHERE email_trabajador = '${email}';`);
        if (trabajador.rows != '') {
